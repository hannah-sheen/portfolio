...
//       const create = () => { build(); if (scrambleCharset) randomizeScrambles(); play(); armHover(); };
...
//   const fallbackFont = useMemo(() => (userHasFont ? {} : {}), [userHasFont]);
//   const commonStyle = useMemo(() => ({ textAlign, ...fallbackFont, ...style }), [textAlign, fallbackFont, style]);
//   const classes = useMemo(() => `${baseTw} ${className}`.trim(), [baseTw, className]);
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  // Synchronize layout state and font readiness
  useEffect(() => {
    if ('fonts' in document) {
      if (document.fonts.status === 'loaded') setFontsLoaded(true);
      else document.fonts.ready.then(() => setFontsLoaded(true));
  ...
  useGSAP(
    () => {
      // 💡 CORE TEARDOWN LOGIC: Strips out split layers and restores clean native HTML text layout
      const teardown = () => {
        if (tlRef.current) { tlRef.current.kill(); tlRef.current = null; }
        if (wrappersRef.current.length) {
          wrappersRef.current.forEach(wrap => {
      ...
      // 💡 FIX FOR WEB-TO-MOBILE: Actively destroys and flushes web styling layers on layout shifts
      if (isMobile) {
        teardown();
        onShuffleComplete?.();
        return;
    ...
    {
      dependencies: [isMobile, text, duration, maxDelay, ease, scrollTriggerStart, fontsLoaded, shuffleDirection, shuffleTimes, animationMode, loop, loopDelay, stagger, scrambleCharset, colorFrom, colorTo, triggerOnce, respectReducedMotion, triggerOnHover, onShuffleComplete],
      scope: ref
    }
  );

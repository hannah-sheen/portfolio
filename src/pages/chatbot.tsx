...
export default function Chatbot({ isOpen, onClose }: ChatbotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'init-1',
      sender: 'bot',
      text: "Hi there! I'm Hannah's portfolio assistant. Want to know more about her backend architecture setups, projects, or technical toolkits? Fire away!"
    }
  ]);
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Server integration failure');
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: crypto.randomUUID(),
          sender: 'bot',
          text: "I ran into a small transmission bump running backend loops. Let's try that question one more time.",
        },
      ]);
    } finally {
      setIsLoading(false);

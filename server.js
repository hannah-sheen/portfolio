...
import { GoogleGenAI } from '@google/genai'; // 💡 Added for chatbot logic
...
// ==========================================
// 💡 CHATBOT ENGINE CONCATENATION LAYER
// ==========================================

// Safeguard against missing keys during initialization crash events
const apiKey = process.env.GEMINI_API_KEY;
if (!apiKey) {
  console.error("CRITICAL CONFIG WARNING: GEMINI_API_KEY environment variable is currently undefined.");
}
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ error: 'Runtime Config Fault: Key environment missing on backend host dashboard.' });
    }
    ...
    console.error('Gemini Execution Crash Details:', error);
    return res.status(500).json({ 
      error: 'Internal processing cluster failure.',
      details: error instanceof Error ? error.message : String(error)
    });
  }
});

// Running fallback tracking logic for local offline testing environments
if (process.env.NODE_ENV !== 'production') {
  const PORT = process.env.PORT || 5000;
  app.listen(PORT, () => console.log(`Backend server running locally on http://localhost:${PORT}`));
}

// Export module configuration directly for Vercel's serverless environment router
export default app;